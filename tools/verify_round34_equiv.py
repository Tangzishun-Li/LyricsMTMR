#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""第 34 轮 A 卡程序化等价性实证：注册表闭包 vs switch 分支逐语句比对。

提取 ItemsParsing.swift 中第 34 轮第五批迁移的 20 个注册表闭包体与对应
switch 分支体，逐语句 diff（末行 `return`/`self =` 前缀归一后比对，
中间语句逐字节比对）。输出 `OK: 20/20 类型注册表闭包与 switch 分支逐行等价`
或逐类型 FAIL 明细。退出码 0=全部等价。

用法：python3 tools/verify_round34_equiv.py
"""
import re
import sys
...
# 第 34 轮第五批迁移 20 类型（形态 A 16 + 形态 B 4），与报告迁移清单一致
BATCH = [
    # 形态 A：全 decodeIfPresent + 默认值
    "breathingGuide", "postureReminder", "travelCountdown", "birthdayCountdown",
    "holidayCountdown", "classCountdown", "ddlList", "readingProgress",
    "noteCapture", "quickScreenshot", "savingsGoal", "taxEstimate",
    "creditCardDue", "ciPipeline", "systemTemp", "diskIO",
    # 形态 B：无参
    "billSplit", "screenPicker", "latexSymbols", "finderTags",
]

lines = open(SRC, encoding="utf-8").read().splitlines()


def find_region(start_pattern, end_pattern):
    """返回 [start_idx, end_idx]（含端点行），未找到返回 None。"""
    start = end = None
    for i, ln in enumerate(lines):
        if start is None and start_pattern in ln:
            start = i
        elif start is not None and end_pattern in ln:
            end = i
            break
...
def extract_closure_bodies():
    ...
    reg = find_region("static let registeredTypeDecoders", "    ]")
    assert reg, "注册表字典区域未找到"
    bodies = {}
    i = reg[0]
    while i <= reg[1]:
        ...
        if m and m.group(1) in BATCH:
            ...
            body = []
            i += 1
            while i <= reg[1] and not lines[i].strip().startswith("},"):
                body.append(lines[i])
                i += 1
...
def extract_switch_bodies():
    ...
    sw = find_region("switch type {", "    }\n")
    # 用行号范围限定：switch type { 所在行 到 下一个同级 }（含 case .opencodeGoUsage 的分支）
    sw_start = next(i for i, ln in enumerate(lines) if "switch type {" in ln and "ItemsParsing" not in ln)
    # 找到该 switch 的闭合：从 sw_start 之后找第一个缩进为 8 空格且为 "}" 的行（与 "        switch type {" 同级）
    sw_end = None
...
def main():
    ...
    for key in BATCH:
        ...
        # 形态 B 无参分支体可能为空（`self = .billSplit` 在 case 行）——等价判定：
        # 闭包体应为 ["return .xxx"]，switch 体应为 []（case 行自带赋值）
        if not s and c == [f"self = .{key}"]:
            results.append((key, True, "无参样板 case 行等价"))
            continue
        ok = c == s
