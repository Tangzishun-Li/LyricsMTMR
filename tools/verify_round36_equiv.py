#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""第 36 轮 A 卡程序化等价性实证：注册表闭包 vs switch 分支逐语句比对（换锚补迁）。

提取 ItemsParsing.swift 中第 36 轮换锚补迁的 1 个注册表闭包体（base64Tool）与对应
switch 分支体，逐语句 diff（末行 `return`/`self =` 前缀归一后比对，
中间语句逐字节比对）。输出 `OK: 1/1 类型注册表闭包与 switch 分支逐行等价`
或逐类型 FAIL 明细。退出码 0=全部等价。

用法：python3 tools/verify_round36_equiv.py
"""
import re
import sys
...
# 第 36 轮换锚补迁 1 类型（形态 A 全部），与报告迁移清单一致；
# base64Tool 原为 switch 回退路径测试锚点，本轮换锚（audioSpectrum）后补迁。
BATCH = [
    "base64Tool",
]
