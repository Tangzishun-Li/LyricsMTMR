...
- **卡拉 OK 逐字高亮**：修复跳字重影（变色进度层与底字错位），逐字时间线现与播放进度精确对齐
- **逐字歌词解析**：修复网易云 YRC/KRC 解析崩溃与时间漂移，QQ/酷狗/本地 `.lrcx` 等多源逐字歌词可正常显示
- **设置界面**：「逐字」徽章改为显示真实解析结果，不再虚标
- **歌词过滤**：增强制作人员（词/曲/编曲/混音等 credit 行）过滤规则
