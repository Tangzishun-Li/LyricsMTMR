...
LyricsMTMR 是一款 macOS Touch Bar 增强工具。它把 LyricsX 的多平台歌词引擎集成进 MTMR 的 Touch Bar 组件体系中，让你在 Touch Bar 上实时看到正在播放歌曲的歌词（支持逐字卡拉 OK 高亮），同时保留了 MTMR 完整的组件生态：99 种内置 widget、可视化布局编辑器、多主题、脚本集成等。
...
3. **自定义组件**：设置 → 编辑器，从左侧元素面板的 99 种 widget 中选择；或使用 AppleScript / Shell 脚本显示任意内容
