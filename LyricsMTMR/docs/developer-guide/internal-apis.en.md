...
The six registration points (line numbers measured in round 35 — #2 decode switch moved +47 lines from the round-30 A registry insertion, another +120 lines from the round-31 A batch migration, another +107 lines from the round-32 A third batch, another +85 lines from the round-33 A fourth batch, another +88 lines from the round-34 A fifth batch, and another +44 lines from the round-35 A sixth (final) batch; the rest unchanged since round 26; paths relative to `LyricsMTMR/MTMR/`):
...
| 2 | `ItemType` decode switch | `Core/ItemsParsing.swift:1096-1494` (`switch type {` :1096, `case .appleScriptTitledButton:` :1097 … `case .opencodeGoUsage:` :1488) | `ItemTypeRaw` → `ItemType` decode branches, parses config params; `ItemType` case list at :293-390 (compiler exhaustiveness keeps this switch in sync with the enum); types hitting the registry (`registeredTypeDecoders` :634-1082, 3 pilot + 20 batch-migrated + 20 third-batch + 20 fourth-batch + 20 fifth-batch + 9 sixth-batch + 1 anchor-swap = 93 keys) are decoded by closure first, switch branches kept as exhaustiveness fallback |
...
// 2. ItemsParsing.swift:1331-1335 → decode switch (within :1096-1494)
case .weatherOutfit:
...
#### 2.3.2 Dictionary-driven decode registry (round-30 A pilot + round-31 A batch migration + round-32 A third batch + round-33 A fourth batch + round-34 A fifth batch + round-35 A sixth/final batch + round-36 A anchor-swap closure + round-37 A fallback-contract completion, optional path)

`ItemType.init(from:)` consults a **dictionary-driven decode registry** before the #2 decode switch (`ItemType.registeredTypeDecoders` in `ItemsParsing.swift`; immutable `static let` dict, key = `ItemTypeRaw`, value = param-parsing closure): hit → closure, miss → switch. Currently **93 types** are registered: the 3 round-30 pilot types (`cpu` / `battery` / `swipe`, covering the three parameter shapes: defaults / parameterless / required-field throwing) plus 20 batch-migrated in round 31 (shape A "all decodeIfPresent + defaults" ×12: `timeButton`/`brightness`/`music`/`pomodoro`/`network`/`upnext`/`lyrics`/`stock`/`usage`/`deepseekBalance`/`networkSpeed`/`uuidGen`; shape B "parameterless" ×6: `volume`/`inputsource`/`nightShift`/`darkMode`/`lyricsTranslate`/`windowSnap`; shape C "required-field decode" ×2: `appleScriptTitledButton`/`shellScriptTitledButton`) plus 20 third-batch-migrated in round 32 (shape A ×14: `dock`/`weather`/`yandexWeather`/`currency`/`playbackProgress`/`quickReply`/`gitStatus`/`apiLatency`/`sshStatus`/`portChecker`/`hashCalc`/`packageTracker`/`foodDelivery`/`weatherOutfit`; shape B ×6: `dnd`/`jsonFormatter`/`timestampConvert`/`httpCodes`/`qrCode`/`readTimer`) plus 20 fourth-batch-migrated in round 33 (shape A ×14: `noiseMeter`/`expenseTracker`/`subscriptionCountdown`/`dailyQuote`/`emailBadge`/`meetingCountdown`/`slackUnread`/`printerStatus`/`standupTimer`/`clipboardHistory`/`wordLookup`/`dockerStatus`/`serverMonitor`/`opencodeGoUsage`; shape B ×6: `regexTester`/`colorConvert`/`regexReference`/`screenLock`/`bluetoothToggle`/`shortcutHints`) plus 20 fifth-batch-migrated in round 34 (shape A ×16: `breathingGuide`/`postureReminder`/`travelCountdown`/`birthdayCountdown`/`holidayCountdown`/`classCountdown`/`ddlList`/`readingProgress`/`noteCapture`/`savingsGoal`/`taxEstimate`/`creditCardDue`/`ciPipeline`/`systemTemp`/`diskIO`/`quickScreenshot`; shape B ×4: `billSplit`/`screenPicker`/`latexSymbols`/`finderTags`) plus 9 sixth/final-batch-migrated in round 35 (shape A ×9: `pixelPet`/`homekitScene`/`aiSelectedText`/`rssUnread`/`citationGen`/`paperProgress`/`paperTags`/`bilibiliFeed`/`apiTester`), closures byte-equivalent to their switch branches; with this batch every migratable branch is migrated (only `base64Tool` stays unmigrated as the rollback-path anchor). Round 36 anchor-swap closure: `base64Tool` migrated (shape A — the former rollback-path test anchor, closures byte-equivalent to their switch branches, programmatic diff tools/verify_round36_equiv.py 1/1), so every migratable branch is now fully migrated (**93 keys**); the **rollback-path test anchor is now `audioSpectrum`** (the only one of the 5 retained classes with real computation — width→barCount density derivation — and no runtime pre-lookup interception, so a registry miss genuinely falls into switch, same shape as base64Tool before the swap). Their switch branches are **kept** (runtime-unreachable but preserve compile-time exhaustiveness; the L2 full-decode assertions keep guarding both paths). Types deliberately kept on the switch branch: `staticButton` (the `unknown` downgrade target, special semantics), `group`/`expandable` (nested recursive decoding), `themeSwitch` (duplicate key pre-registered in `SupportedTypesHolder` — runtime is intercepted by `lookup` first, the `ItemType` branch is only test-reachable, zero benefit), `audioSpectrum` (width→barCount density derivation with comment semantics, now also the rollback-path test anchor). The migration contract is pinned by `MTMRTests/ItemTypeDecodeRegistryTests.swift` (173 hand-written cases — do NOT merge into the generated file): exactly-93 registered keys / field-level equivalence (defaults + explicit values) / unregistered types still decode via switch / missing required field degrades to `unknown`. Round-37 A completed the switch-path contracts for 4 of the 5 retained classes (staticButton required-title passthrough + missing-title downgrade / group nested-items recursion + missing-items downgrade / expandable defaults + explicit values / themeSwitch default `[]` + explicit array; 165→173 cases; audioSpectrum already has the rollback-anchor case, not duplicated) — all 5 retained branches are now pinned by switch-path contract tests, upgrading the exhaustive fallback from a compile-time guarantee to runtime behavior assertions. Evaluation & selection rationale: 《评估报告_第30轮_注册表混合架构decode迁移评估.md》, 《验证报告_第31轮_decode迁移扩大化.md》, 《验证报告_第32轮_decode迁移第三批.md》, 《验证报告_第33轮_decode迁移第四批.md》 and 《验证报告_第34轮_decode迁移第五批.md》 and 《验证报告_第35轮_decode迁移第六批.md》 and 《验证报告_第36轮_decode迁移最终收官.md》 and 《验证报告_第37轮_switch兜底契约补齐.md》 at repo root. New types still follow the six-point flow above; migrating param parsing into the registry is **optional** — skipping it changes nothing, and any migration is double-guarded by reconciliation L2 + equivalence tests.
