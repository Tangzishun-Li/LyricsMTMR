...
The six registration points (line numbers measured in round 32 — #2 decode switch moved +47 lines from the round-30 A registry insertion, another +120 lines from the round-31 A batch migration, and another +107 lines from the round-32 A third batch; the rest unchanged since round 26; paths relative to `LyricsMTMR/MTMR/`):
...
| 2 | `ItemType` decode switch | `Core/ItemsParsing.swift:870-1268` (`switch type {` :870, `case .appleScriptTitledButton:` :871 … `case .opencodeGoUsage:` :1262) | `ItemTypeRaw` → `ItemType` decode branches, parses config params; `ItemType` case list at :293-390 (compiler exhaustiveness keeps this switch in sync with the enum); types hitting the registry (`registeredTypeDecoders` :627-856, 3 pilot + 20 batch-migrated + 20 third-batch = 43 keys) are decoded by closure first, switch branches kept as exhaustiveness fallback |
...
// 2. ItemsParsing.swift:1105-1109 → decode switch (within :870-1268)
case .weatherOutfit:
