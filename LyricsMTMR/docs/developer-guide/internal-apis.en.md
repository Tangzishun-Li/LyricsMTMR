...
| 2 | `ItemType` decode switch | `Core/ItemsParsing.swift:1087-1485` (`switch type {` :1087, `case .appleScriptTitledButton:` :1088 … `case .opencodeGoUsage:` :1479) | `ItemTypeRaw` → `ItemType` decode branches, parses config params; `ItemType` case list at :293-390 (compiler exhaustiveness keeps this switch in sync with the enum); types hitting the registry (`registeredTypeDecoders` :630-1073, 3 pilot + 20 batch-migrated + 20 third-batch + 20 fourth-batch + 20 fifth-batch + 9 sixth-batch = 92 keys) are decoded by closure first, switch branches kept as exhaustiveness fallback |
...
// 2. ItemsParsing.swift:1322-1326 → decode switch (within :1087-1485)
case .weatherOutfit:
