...
The six registration points (line numbers measured in round 34 — #2 decode switch moved +47 lines from the round-30 A registry insertion, another +120 lines from the round-31 A batch migration, another +107 lines from the round-32 A third batch, another +85 lines from the round-33 A fourth batch, and another +88 lines from the round-34 A fifth batch; the rest unchanged since round 26; paths relative to `LyricsMTMR/MTMR/`):
...
| 2 | `ItemType` decode switch | `Core/ItemsParsing.swift:1043-1441` (`switch type {` :1043, `case .appleScriptTitledButton:` :1044 … `case .opencodeGoUsage:` :1435) | `ItemTypeRaw` → `ItemType` decode branches, parses config params; `ItemType` case list at :293-390 (compiler exhaustiveness keeps this switch in sync with the enum); types hitting the registry (`registeredTypeDecoders` :627-1029, 3 pilot + 20 batch-migrated + 20 third-batch + 20 fourth-batch + 20 fifth-batch = 83 keys) are decoded by closure first, switch branches kept as exhaustiveness fallback |
...
// 2. ItemsParsing.swift:1278-1282 → decode switch (within :1043-1441)
case .weatherOutfit:
