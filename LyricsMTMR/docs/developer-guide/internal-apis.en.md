...
The six registration points (line numbers measured in round 33 — #2 decode switch moved +47 lines from the round-30 A registry insertion, another +120 lines from the round-31 A batch migration, another +107 lines from the round-32 A third batch, and another +85 lines from the round-33 A fourth batch; the rest unchanged since round 26; paths relative to `LyricsMTMR/MTMR/`):
...
| 2 | `ItemType` decode switch | `Core/ItemsParsing.swift:955-1353` (`switch type {` :955, `case .appleScriptTitledButton:` :956 … `case .opencodeGoUsage:` :1347) | `ItemTypeRaw` → `ItemType` decode branches, parses config params; `ItemType` case list at :293-390 (compiler exhaustiveness keeps this switch in sync with the enum); types hitting the registry (`registeredTypeDecoders` :627-941, 3 pilot + 20 batch-migrated + 20 third-batch + 20 fourth-batch = 63 keys) are decoded by closure first, switch branches kept as exhaustiveness fallback |
...
// 2. ItemsParsing.swift:1190-1194 → decode switch (within :955-1353)
case .weatherOutfit:
