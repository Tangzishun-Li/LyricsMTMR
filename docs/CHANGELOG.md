# LyricsMTMR 更新日志（详细历史）

> 本文件是 README 中"更新日志"段（v0.3 / v0.2 / v0.1）的**完整详细历史**。
> 包含每个迭代轮次（R-N）的技术细节、commit 引用、报告链接。
> 摘要版（用户感知）请见 [README.md](../README.md#-更新日志)。
...
> 承接第 63 轮：EditorTabView 死代码簇删除（用户决策 A 落地）+ EditorSchema 编译提速 P1 首刀；整体构建 SUCCEEDED、全量回归 R63 到轮触发 685 用例 0 失败、锚点巡检连续第四十四轮 0 ERROR。第 64 轮方向待定（候选见 docs/轮次速查.md）。

#### 新增

- **EditorSchema 注册完整性测试（r63-b，commit 7d917c9）**：EditorSchemaRegistryIntegrityTests 4 用例——97 类型不重不漏/palette 引用集与注册集全等/注册完整性/std 65 类型 width+align 显示分区，守住 items 元数据契约
- **EditorSchema 拆分校验工具（r63-b）**：scripts/r63b_split_items.py（按 palette 分区拆分生成）+ scripts/r63b_verify_literal.py（297 条数据行逐字等价校验零漂移）

#### 工程与稳定性

- **EditorTabView 死代码簇删除（r63-a，commit f2561a9，用户决策 A 落地）**：git rm EditorTabView.swift(470)/ElementPaletteView.swift(364)/TouchBarPreviewView.swift(400)/PropertiesInspectorView.swift(449) 净 -1683 行；project.pbxproj 摘除恰 16 行注册（PBXBuildFile×4/PBXFileReference×4/group children×4/Sources phase×4，脚本断言零残留）；file-structure.zh.md:81 注释同步；全仓四类构造调用 grep 零命中
- **EditorSchema 类型检查治理 P1 首刀（r63-b，commit 7d917c9）**：items 单巨表达式按 21 个 palette 分区拆为 partXxx() 显式类型分段构造，297 条数据行类型序/key 序/字面零漂移——R54 定位的编译期负担结构性减负，运行时语义零变化
- **工程版本号对齐**：Info.plist 0.62/487 → 0.63/488（第 63 轮收口落地，INTEG 版本统一动作延续）
