...
- **上交流程**：以官方通知原文逐日核对 → 在代码注释中记录文号 + URL
  （参见 `StockBarItem.swift:369-370` 的 2026 写法）→ 提交 PR 合入 main。
- **代码位置**：`LyricsMTMR/MTMR/Widgets/Life/StockBarItem.swift`
  - `aShareHolidays`（休市日，含假期窗口内周末，便于整窗核对）：`:375-399`
  - `aShareMakeupDates`（调休补班日，周末上班视为交易日）：`:404-419`
...
3. **更新测试锚点**：`LyricsMTMR/MTMRTests/StockMarketHoursTests.swift` 为 ITER-8 表驱动断言，
   直接从两表生成断言——**改表后跑测试即自动覆盖**；若后续 ITER-12「官方锚点金丝雀」
   （固定日期断言）落地，需同步核对锚点日期是否仍属于官方安排。
